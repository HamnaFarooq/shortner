...
// 		Main::set("url", "{$this->config["url"]}/user/cash_out");
		...
		if (isset($_POST["token"])) {
			...
			$array = array(
				...
				// 		Main::set("url", "{$this->config["url"]}/user/cash_out");
					...
					{
					   // print_r($this->config["pt"]);
					   // print_r($this->config["pppublic"]);
					   // die();
					   
						if ($this->admin()) return Main::redirect(Main::href("user", "", FALSE), array("danger", e("Wow there. You are an admin. You can't cash out for the credits..")));
				
						if (isset($_POST["token"])) {
							if (!Main::validate_csrf_token($_POST["token"])) {
								return Main::redirect(Main::href("user", "", FALSE), array("danger", e("Something went wrong, please try again.")));
							}
							$user = $this->db->get("user", ["id" => $this->user->id], ["limit" => 1]);
							if (!Main::validate_pass($_POST["password"], $user->password)) {
								return Main::redirect(Main::href("user", "", FALSE), array("danger", e("Your password is incorrect.")));
							}
							
							if ($_POST["email"]) {
								return Main::redirect(Main::href("user", "", FALSE), array("danger", e("Please enter an email address..")));
							}
							
							
							if (empty($this->config["pppublic"]) && empty($this->config["ppprivate"]) ) {
								print_r("lalalalaalalalalaal ");
								return Main::redirect(Main::href("user", "", FALSE), array("danger", e("We may have an issue with our paypal account. Please contact us")));
							}
							...
							// $AYPAL_SECRET = 'EIHIHy_71wHqKy-e5I6pFINpAJwFbY5XPwt6-hi_8zjZEWWhScYNwdZPQsXOcLzKoBpBMWfJzsg-dtyx';
							// $PAYPAL_CLIENT_ID = 'AVLB8-JHfGJUSlJjs3sg2qSmirjFNZzlICouxrfoEyZjEm0rrXjeBBmcvrVHjCQ8cSGGl1mFJ8BPFF8M';
							
							$AYPAL_SECRET = $this->config["ppprivate"];
							$PAYPAL_CLIENT_ID = $this->config["pppublic"];
							...
							$post_vars = "cmd=_notify-synch&tx=" . $tid . "&at=" . $auth_token;
								...
								curl_setopt($ch, CURLOPT_URL, "https://api.sandbox.paypal.com/v1/payments/payouts?sync_mode=false");    //DUMMY
								curl_setopt($ch, CURLOPT_URL, $url);
								curl_setopt($ch, CURLOPT_POST, 1);
								curl_setopt($ch, CURLOPT_POSTFIELDS, $post_vars);
								curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
								curl_setopt($ch, CURLOPT_POSTFIELDS, "{\"sender_batch_header\":{\"sender_batch_id\":\"$sender_batch_id\",\"email_subject\":\"You have a payout!\",\"recipient_type\":\"EMAIL\"},\"items\":[{\"recipient_type\":\"EMAIL\",\"amount\":{\"value\":$Amount,\"currency\":\"USD\"},\"note\":\"Thanks for your patronage!\",\"sender_item_id\":\"$sender_item_id\",\"receiver\":$email }]}");  //DUMMY
								...
								$this->db->update("user", ["credits" => 0], ["id" => $user->id]);
					
								return Main::redirect("user", array("success", e("Your Credits are sent to your account.")));
								
							}
							else{
								print_r(" jkfhrskhun");
								return Main::redirect(Main::href("user", "", FALSE), array("danger", e("You cannot Cash out in a Triel / Free account.")));
							}
							print_r(" 843whafdjknhekzj");
							return Main::redirect(Main::href("user", "", FALSE), array("danger", e("There was some issue with the payment transfer")));
						}
						return Main::redirect(Main::href("user", "", FALSE), array("danger", e("Something went wrong, please try again.")));
					}
					/**
